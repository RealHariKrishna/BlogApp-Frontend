...
const CropEasy = ({ photo, setOpenCrop }) => {
  ...
  const handleCropComplete = (cropedArea, croppedAreaPixels) => {
    setCroppedAreaPixels(croppedAreaPixels);
  };

  const handleCropImage = async () => {
    try {
      const cropedImage = await getCroppedImg(photo?.url, croppedAreaPixels);

      const file = new File([cropedImage.file], `${photo?.file?.name}`, {
      ...
      <div className="bg-white h-fit w-full sm:max-w-[350px] p-5 rounded-lg">
        ...
        <div>
          ...
          <input
            ...
            className="w-full mb-6 h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer range-sm"
          />
        </div>
