            ...
            <ul className="bg-dark-soft lg:bg-transparent text-center flex flex-col shadow-lg rounded-lg overflow-hidden">
              {item.items.map((page, index) => (
                <Link
                  key={index}
                  to={page.href}
                  className="hover:bg-dark-hard hover:text-white px-4 py-1 my-2 text-center text-white lg:text-dark-soft"
                >
                  {page.title}
                </Link>
              ))}
    ...
    <section className="sticky top-0 left-0 right-0 z-50 bg-white">
      <header className="container mx-auto px-5 flex justify-between py-4 items-center">
        ...
        <div
          ...
          } transition-all duration-300 mt-[56px] lg:mt-0 text-white lg:text-dark-soft bg-dark-hard lg:bg-transparent z-[49] flex flex-col w-full lg:w-auto justify-center lg:justify-end lg:flex-row lg:static fixed top-0 bottom-0 gap-x-9 items-center`}
        >
          <ul className="flex flex-col text-white lg:text-dark-soft lg:flex-row items-center gap-y-5 gap-x-2 font-semibold">
            {NavItemsInfo.map((item) => (
              <NavItem key={item.name} item={item} />
            ))}
          </ul>
          {userState.userInfo ? (
            <div className="flex flex-col text-white lg:text-dark-soft lg:flex-row items-center gap-y-5 gap-x-2 font-semibold">
